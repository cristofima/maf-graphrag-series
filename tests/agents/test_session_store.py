...
def test_ttl_expiration_removes_stale_session_and_counts_metric() -> None:
    clock = _Clock(10.0)
    ...
    record, created = store.get_or_create(key.session_id)

    assert created is True
    assert store.get_record(key.session_id) is record

    clock.advance(6)
    assert store.get_record(key.session_id) is None
    assert store.metrics.ttl_expirations == 1


def test_capacity_eviction_removes_oldest_record() -> None:
    clock = _Clock(0.0)
    store = InMemorySessionStore(
        ttl_seconds=100,
        max_count=2,
    ...
    store.get_or_create(key1.session_id)
    clock.advance(1)
    store.get_or_create(key2.session_id)
    clock.advance(1)
    store.get_or_create(key3.session_id)

    assert store.get_record(key1.session_id) is None
    assert store.get_record(key2.session_id) is not None
    assert store.get_record(key3.session_id) is not None
    assert store.metrics.evictions == 1


def test_cleanup_interval_runs_and_updates_cleanup_metric() -> None:
    clock = _Clock(0.0)
    store = InMemorySessionStore(
        ttl_seconds=2,
        max_count=10,
    ...
    store.get_or_create(key.session_id)
    clock.advance(2)
...
def test_append_turn_applies_sliding_window_and_compaction_diagnostics() -> None:
    store = InMemorySessionStore(
        ttl_seconds=100,
        max_count=10,
        cleanup_interval_seconds=100,
    ...
    key = SessionKey.create(channel_id="msteams", conversation_id="c1", user_id="u1")
    record, _ = store.get_or_create(key.session_id)
